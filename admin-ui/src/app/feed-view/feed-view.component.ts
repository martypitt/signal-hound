                ...
                <tr *ngFor="let endpoint of apiEndpoints">
                  ...
                  <td class="text-sm">
                    <div class="flex items-center">
                      {{ endpoint.value }}
                      <tui-svg class="ml-4 cursor-pointer text-gray-400 group-hover:text-indigo-600" src="tuiIconCopy"
                               (click)="copyToClipboard(endpoint.value)"></tui-svg>
                    </div>
                  </td>
                </tr>
              </table>
            ...
            <ng-container *ngFor="let col of feedSpec.extractions">
              <td *tuiCell="col.title">
                <ng-container *ngIf="row.success">
                  {{ unwrapArrays(row.signals[col.title])}}
                </ng-container>
              </td>
            </ng-container>
...
export class FeedViewComponent {
  ...
  unwrapArrays(signal: any) {
    ...
    if (typeof signal !== 'object') {
      ...
      return JSON.stringify(signal, null, 3)
    }
  }
}
